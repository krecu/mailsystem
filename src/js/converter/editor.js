...
var _propInput = function(model, prop, propAccessor, editType) {
  ...
  if (widget == 'boolean') {
    ...
    //html += '<div class="ui-textbutton">';
    //html += '<select>';
    //html += '<option>forbes.ru</option>';
    //html += '<option>other.ru</option>';
    //html += '</select>';
    //html += '<input class="ui-textbutton-input" type="text" id="forbes" data-bind="jqAuto: { source: $root.forbes.forbesExternalData(), value: ko.observable() }"/>';
    //"jqAuto: { source: remoteHandler,                     value: remoteValue,     inputProp: 'test',   labelProp: 'test',   valueProp: 'val',   options: { delay: 250 } }"
    //html += '<a class="ui-textbutton-button" data-bind="visible: typeof $root.linkDialog !== \'undefined\', click: typeof $root.linkDialog !== \'undefined\' ? $root.linkDialog.bind($element.previousSibling) : false, button: { icons: { primary: \'fa fa-fw fa-ellipsis-h\' }, label: \'Opzioni\', text: false }">Opzioni</a>';
    //html += '</div>';
    //html += '<input type="text" id="name-search" data-bind="select2"/>';
    html += '<div><select style="width: 100%" multiple="true" data-bind="options: $root.forbesValues(), selectedOptions: $root.forbesSelectedValue, select2: { placeholder: \'pick some colors\', multiple: false, ajax: $root.forbesAjaxOptions, escapeMarkup: function (markup) { return markup; }, minimumInputLength: 1, templateResult: $root.formatRepo, templateSelection: $root.formatRepoSelection }"></select></div>';
    html += '<div class="ui-textbutton">';
    html += '<a class="ui-textbutton-button" data-bind="click: $root.forbesUpdateElem($data)">Применить</a>';
    html += '</div>';
    //html += '<span data-bind="text: $root.forbesGetNode($root.forbesSelectedValue())"></span>';
