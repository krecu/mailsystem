...
function initializeEditor(content, blockDefs, basePath, galleryUrl) {
    ...
    /** CREATE NEW FUNCTIONS FOR FORBES NODE */

    viewModel.forbesValues = ko.observableArray([]);
    viewModel.forbesGetNode = function(id){
        var items = viewModel.forbesValues();
        if (items.length === 0 || id === undefined) return "";

        if (id.constructor == Array && (!isNaN(parseFloat(id[0])) && isFinite(id[0]))) {
            return items[id[0]];
        }
        return null;
    };
    viewModel.forbesUpdateElem = function(elem){
        var id = viewModel.forbesSelectedValue();
        var node = viewModel.forbesGetNode(id);
        var block = viewModel.selectedBlock();
        if (elem == block) {
            if (node !== undefined || node !== null) {
                elem.materialTitle(node.title);
                elem.materialTeaser(node.desc);
                elem.materialCategory(node.term);
                elem.materialUrl(node.uri);
                elem.materialImage(node.img);
            }
        }
    };
    viewModel.forbesSelectedValue = ko.observable("(none)");
    viewModel.forbesAjaxOptions = {
        url: "/forbes.json",
        ...
        processResults: function (data, params) {
            viewModel.forbesValues(data.items);
            return {
                results: data.items
            };
        },
    ...
    viewModel.formatRepo = function(node) {
        if (node.loading) return node.text;

        return "<div class='select2-result-node'>" +
            "<div class='select2-result-node-img'><img src='" + node.img + "' /></div>" +
            "<div class='select2-result-node-title'><a href='" + node.uri + "'>" + node.title + "</a></div>" +
            "<div class='select2-result-node-desc'>Описание: " + node.desc + "</div>" +
            "<div class='select2-result-node-term'>Категория: " + node.term + "</div>" +
        "</div>";
    };

    viewModel.formatRepoSelection = function(node) {
